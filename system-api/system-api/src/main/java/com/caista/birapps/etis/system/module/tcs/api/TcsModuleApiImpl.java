...
import com.caista.birapps.etis.system.module.tcs.TcsConfig;
import com.caista.birapps.etis.system.module.tcs.TcsModule;
import com.caista.birapps.etis.system.module.tcs.TcsModuleFactory;
...
/**
 * The Class TcsModuleApiImpl.
 */
@RestController
@RequestMapping("/tcsmodules")
@Api(description = "API for TCS Module", produces = "application/json")
public class TcsModuleApiImpl implements TcsModuleApi {

	/** The tcs module factory. */
	@Autowired
	private TcsModuleFactory tcsModuleFactory;

	/** The tcs config. */
	@Autowired
	private TcsConfig tcsConfig;
	...
	/** The Constant LOGGER. */
	private static final Logger LOGGER = LoggerFactory.getLogger(TcsModuleApiImpl.class);

	@Override
	@ApiOperation(value = "Get all Tcs modules based on authentication token", response = TcsModule.class)
	...
	public ResponseEntity<List<Module>> getPermission(@RequestHeader HttpHeaders headers) {
		...
		try {
			...
			LOGGER.info("User with token :" + token + " has no access to " + tcsConfig.getClientid());
	...
	private List<Module> getUserModulesByPermission(String token) throws KeycloakEtisUnauthorizedException {
		...
		try {
			String rpt = keycloakEtisAuthorizationService.getUserAuthorization(token, tcsConfig.getClientid());
			Map<String, Permission> permissions = keycloakEtisAuthorizationService.buildPermissionMap(rpt,
					tcsConfig.getClientid());
			if (CollectionUtils.isEmpty(permissions)) {
				throw new KeycloakEtisUnauthorizedException("Unauthorized access! no role for other modules.");
			}
			modules = tcsModuleFactory.create().getModules();
			traverse(modules, permissions);
			return modules;

		} catch (AuthorizationDeniedException e) {
			LOGGER.warn("User with token :" + token + " has no access to " + tcsConfig.getClientid());
